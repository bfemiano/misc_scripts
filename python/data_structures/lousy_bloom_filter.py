class LousyBloomFilter(object):
    ...
    def __init__(self, capacity):
        self.bit_vector = [0 for _ in range(capacity)]
        self.capacity = capacity

    def add(self, hash):
        self.bit_vector[hash % self.capacity] = 1

    def contains(self, hash):
        return bool(self.bit_vector[hash % self.capacity])

def test_no_collisions_means_100perc_accuracy():
    bloom = LousyBloomFilter(5)
    bloom.add(0)
    bloom.add(4)
    assert bloom.bit_vector == [1, 0, 0, 0, 1]
...
def test_false_positives_when_collisions():
    ...
    bloom.add(0)
    bloom.add(6)
    assert bloom.bit_vector == [1, 1, 0, 0, 0] # 6 was indexed at 1 in the vector, so checking for 1 will now yield a false positive.
    assert bloom.contains(0)
