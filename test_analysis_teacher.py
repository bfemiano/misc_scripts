...
class SparkEmployeeAnalysisTest(unittest2.TestCase):
    ...
    def test_rdd_num_engs_with_3_or_more_years(self):
        ...
        '''
        output = self.session.sparkContext.parallelize(self.test_data)\
            .filter(lambda (name, dept, years): dept == 'engineering' and years >= 3)\
            .count()
    ...
    def test_dataframe_num_employees_per_dept(self):
        ...
        '''
        data = self.session.sparkContext.parallelize(self.test_data)\
            .map(lambda x: Row(name=str(x[0]), dept=str(x[1]), years=int(x[2])))
        df = self.session.createDataFrame(data)
        df.registerTempTable("employees")
        output = self.session.sql("SELECT dept, COUNT(*) FROM employees GROUP BY dept").rdd.collect()
        expected = [
            ("engineering", 10),
            ("marketing", 5),
            ("sales", 5),
